...
- The random variables that we study will come in one of two varieties,
...
Yes. $f(x) \geq 0$ for all $x$, and the area under the pdf is $\frac{1}{2} (0.0 + 2.0) = 1.0$.
...
pbeta(.75, 2, 1)
```
A more generic approach that will become useful when dealing with more
complex pdf's is to define the pdf function and use R's built-in `integrate`
method.
```{r}
PDF <- function(x){
  ifelse(x > 0 & x < 1,  
...
integrate(f = PDF, lower = 0, upper = 0.75)
```
---
